...
data2 = pd.get_dummies(data, columns=["Stall_no","Market_Category","Loyalty_customer","Product_Category","Grade","Discount_avail"], drop_first=True)
        # print(df_encoded]
train2= data2[data2.Selling_Price != -1000000000].reset_index(drop=True)
test2 = data2[data2.Selling_Price == -1000000000].reset_index(drop=True)
...
testx=test2.drop(columns=['Selling_Price'])



ty=train2["Selling_Price"].astype(float)
tx=train2.drop(columns=['Selling_Price'])
...
#  n_jobs=-1
#  )
model=ensemble.GradientBoostingRegressor(n_estimators=40,max_depth=50,min_samples_split=2,learning_rate=0.1,loss='ls')
# model=ensemble.GradientBoostingRegressor(n_estimators=170,max_depth=8,min_samples_split=2,learning_rate=0.05,loss='ls')
...
# model=sklearn.neighbors.KNeighborsRegressor(n_neighbors=10, weights='uniform', algorithm='auto', leaf_size=30, p=2, metric='minkowski', metric_params=None, n_jobs=None)
