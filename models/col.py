...
print(pd.DataFrame(train["Selling_Price"]).info())
...
# train=train[train["Maximum_price"]<=48000]
...
# train=train[(train["Selling_Price"]<train["Maximum_price"])]
# train["Selling_Price"]=train["Selling_Price"].fillna(train["Selling_Price"].mean())
...
for feat in ["Stall_no","Market_Category","Loyalty_customer","Product_Category","Grade","Discount_avail","Product_Category_Grade","charges_2 (%)_Grade"]:
    lbl_enc = preprocessing.LabelEncoder()
    temp_col = data[feat].fillna("Rare").astype(str).values
    data.loc[:, feat] = lbl_enc.fit_transform(temp_col)
data=data[["Stall_no","Market_Category","Loyalty_customer","Product_Category","Grade","Demand","Discount_avail","charges_1","charges_2 (%)","Minimum_price","Maximum_price","charges_range","Product_Category_Grade","Selling_Price","charges_2 (%)_Grade"]]
import numpy as np
...
# print(data[:0])
# print(data.shape)
data=pd.DataFrame(data,columns=["Stall_no","Market_Category","Loyalty_customer","Product_Category","Grade","Demand","Discount_avail","charges_1","charges_2 (%)","Minimum_price","Maximum_price","charges_range","Product_Category_Grade","Selling_Price","charges_2 (%)_Grade"])
import matplotlib.pyplot as plt 
# al=[i for i in range(0,9703)]
# data = data.astype({"charges_1":'int', "charges_2 (%)":'int'})  
# plt.scatter(al,data["charges_2 (%)"])
# plt.show()
# plt.scatter(al,data["charges_1"])
# plt.show()
# plt.scatter(al,data["Demand"])
# plt.show()
# plt.scatter(al,data["Minimum_price"])
# plt.show()
# plt.scatter(al,data["Maximum_price"])
# plt.show()
...
# train2=train2[((train2["Selling_Price"]<train2["Minimum_price"]) & (train2["Discount_avail"]==1.0)) |((train2["Selling_Price"]>train2["Minimum_price"]) & (train2["Discount_avail"]==0.0))]
# 
# print(train2.info())
# print(test2.info())
...
testx=test2[["Stall_no","Market_Category","Loyalty_customer","Product_Category","Grade","Demand","Discount_avail","charges_1","charges_2 (%)","Minimum_price","Maximum_price","charges_range","Product_Category_Grade","charges_2 (%)_Grade"]]




tx=train2[["Stall_no","Market_Category","Loyalty_customer","Product_Category","Grade","Demand","Discount_avail","charges_1","charges_2 (%)","Minimum_price","Maximum_price","charges_range","Product_Category_Grade","charges_2 (%)_Grade"]]
...
#  n_jobs=-1
#  )
# model=ensemble.GradientBoostingRegressor(n_estimators=185,max_depth=7,min_samples_split=2,learning_rate=0.11,loss='ls')
# model=ensemble.GradientBoostingRegressor(n_estimators=918,max_depth=7,min_samples_split=2,learning_rate=0.026,loss='ls',random_state=48)
# model=ensemble.GradientBoostingRegressor(n_estimators=908,max_depth=7,min_samples_split=2,learning_rate=0.026,loss='ls',random_state=48)
# model=ensemble.GradientBoostingRegressor(n_estimators=998,max_depth=7,min_samples_split=2,learning_rate=0.026,loss='ls',random_state=48)
...
dat["Selling_Price"]=d
dat.to_csv("xgnew2.csv",index=False,)